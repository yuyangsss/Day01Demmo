...
public class CyclicBarrierDemo {
    public static void main(String[] args) {
         ...
         */
        CyclicBarrier cyclicBarrier=new CyclicBarrier(5,new MesstingRunnable());
        for (int i=1;i<=5;i++){
            new PeopleThread("员工："+i,cyclicBarrier).start();
        }
    }
}
//任务类：开始开会的任务
class MesstingRunnable implements Runnable{
    @Override
    public void run(){
        System.out.println("人员到齐"+Thread.currentThread().getName()+"主持会议");
    }
}
//员工类
class PeopleThread extends Thread{
    private CyclicBarrier cyclicBarrier;
    public PeopleThread(String name,CyclicBarrier cyclicBarrier){
        super(name);
    ...
    public void run(){
        try{
          ...
          cyclicBarrier.await();//自己做完了，告诉循环屏障我结束了！
        }catch (Exception e){
            e.printStackTrace();
        }
    }
