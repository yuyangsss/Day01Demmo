...
public class VolatileAtomicThread {
    public static void main(String[] args) {
        //创建VolatileAtomicThreda任务对象
        Runnable target=new VolatileAtomicRunnable();
        //开启100个线程对执行这个一个任务。
        for(int x=1;x<=100;x++){
...
class VolatileAtomicRunnable implements Runnable{
    //原子类中封装好了整型变量，默认值是0
    ...
    public void run(){
        //对该变量进行++操作，100次
        for(int x=1;x<=100;x++){
            int count=atomicInteger.incrementAndGet();//底层变量+1并且返回！
